...
function usageStatus(item) {
  if (item.checkStatus === 'error') return 'error';
  const used = Number(item.usedWebspace);
  const allocated = Number(item.allocatedWebspace);
  if (!Number.isFinite(used) || !Number.isFinite(allocated) || allocated <= 0) return 'unknown';
