...
export default function AddApplicationPage() {
  ...
  const { notify } = usePm2();
  const [form, setForm] = useState({ name: '', script: '', cwd: '', interpreter: 'node', mode: 'fork', instances: 1, environment: 'production', nodeEnv: 'production', port: '5001' });
  ...
  const submit = () => {
    if (!form.name || !form.script || !form.cwd) { setError('Please complete all required fields.'); return; }
    setError('');
    notify('Application configuration created successfully.');
  ...
  return <Grid container rowSpacing={3}>
    ...
    <Grid size={12}><Stack direction="row" spacing={1} sx={{ justifyContent: 'flex-end' }}><Button component="button" onClick={() => navigate('/applications')}>Cancel</Button><Button variant="contained" onClick={submit}>Start Application</Button></Stack></Grid>
  </Grid>;
}
