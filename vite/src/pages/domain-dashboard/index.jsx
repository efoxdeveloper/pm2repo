...
const emptyData = {
  summary: { total: 0, healthy: 0, attention: 0, errors: 0, sslEnabled: 0, sslHostsTotal: 0, sslHostsHealthy: 0, sslHostsAttention: 0, sslHostsErrors: 0, sslExpiring: 0, sslExpired: 0, domainExpiring: 0, domainExpired: 0, allocatedWebspaceTracked: 0, webspaceDirectories: 0, webspaceErrors: 0, totalAllocatedWebspace: 0, usedWebspaceTracked: 0, totalUsedWebspace: 0, autoRenewal: 0, scansToday: 0 },
  ...
  return (
    <TableContainer>
      <Table size="small">
        ...
        <TableBody>
          {!domains.length && <TableRow><TableCell colSpan={4}><Typography color="text.secondary" sx={{ py: 2 }}>{emptyMessage}</Typography></TableCell></TableRow>}
          {domains.map((item) => (
...
export default function DomainDashboardPage() {
  ...
  const webspacePercent = summary.totalAllocatedWebspace ? Math.min(100, Math.round((summary.totalUsedWebspace / summary.totalAllocatedWebspace) * 100)) : 0;
  const sslPercent = summary.sslHostsTotal ? Math.round((summary.sslHostsHealthy / summary.sslHostsTotal) * 100) : 0;
    ...
    <Grid container rowSpacing={3} columnSpacing={2.75}>
      <Grid size={12}>
        <Stack direction={{ xs: 'column', md: 'row' }} sx={{ alignItems: { md: 'center' }, justifyContent: 'space-between', gap: 1 }}>
          <Box>
            <Typography variant="h5">Website &amp; Hosting Overview</Typography>
            <Typography variant="body2" color="text.secondary">One operational view for primary domains, SSL hosts, and multi-directory webspace.</Typography>
          </Box>
          <Stack direction="row" spacing={1}>
            {hasPermission('domains.view') && <Button variant="outlined" onClick={() => navigate('/domains')}>Domains</Button>}
      ...
      <Grid size={{ xs: 12, sm: 6, lg: 3 }}><MetricCard title="SSL hosts monitored" value={loading ? '…' : summary.sslHostsTotal} helper={`${summary.sslHostsAttention + summary.sslHostsErrors} need attention`} icon={<SafetyCertificateOutlined />} color="success.main" /></Grid>
      <Grid size={{ xs: 12, sm: 6, lg: 3 }}><MetricCard title="Webspace directories" value={loading ? '…' : summary.webspaceDirectories} helper={`${summary.webspaceErrors || 0} check errors`} icon={<HddOutlined />} color="warning.main" /></Grid>
      <Grid size={{ xs: 12, sm: 6, lg: 3 }}><MetricCard title="Domains expiring within 30 days" value={loading ? '…' : summary.domainExpiring} helper={`${summary.domainExpired} already expired`} icon={<CalendarOutlined />} color="error.main" /></Grid>
      ...
      <Grid size={{ xs: 12, md: 5 }}>
        <MainCard title="Coverage snapshot">
          <Stack spacing={2}>
            <Box><Stack direction="row" sx={{ justifyContent: 'space-between' }}><Typography variant="body2">Healthy SSL hosts</Typography><Typography variant="body2" fontWeight={600}>{summary.sslHostsHealthy}/{summary.sslHostsTotal}</Typography></Stack><LinearProgress variant="determinate" value={sslPercent} color="success" sx={{ mt: 0.75 }} /></Box>
            <Box><Stack direction="row" sx={{ justifyContent: 'space-between' }}><Typography variant="body2">Used / allocated webspace</Typography><Typography variant="body2" fontWeight={600}>{summary.totalUsedWebspace.toFixed(2)} / {summary.totalAllocatedWebspace.toFixed(2)} GB</Typography></Stack><LinearProgress variant="determinate" value={webspacePercent} sx={{ mt: 0.75 }} /></Box>
      ...
      <Grid size={12}>
        <MainCard title="Recent primary-domain checks" content={false}>
          <TableContainer>
            <Table size="small">
              <TableHead><TableRow><TableCell>Domain</TableCell><TableCell>Last checked</TableCell><TableCell>Status</TableCell><TableCell>Domain expiry</TableCell></TableRow></TableHead>
              <TableBody>
                {!domains.recent.length && <TableRow><TableCell colSpan={4}><Typography color="text.secondary" sx={{ py: 2 }}>No domain checks available.</Typography></TableCell></TableRow>}
                {domains.recent.map((item) => <TableRow key={item.domain} hover><TableCell><Typography variant="subtitle2">{item.domain}</Typography></TableCell><TableCell>{formatDate(item.scannedAt)}</TableCell><TableCell><Chip size="small" variant="combined" color={statusColor(item.status)} label={item.status || 'unknown'} /></TableCell><TableCell><ExpiryValue value={item.registration?.daysRemaining} /></TableCell></TableRow>)}
              </TableBody>
            </Table>
          </TableContainer>
        </MainCard>
      ...
      <Grid size={12}><Stack direction="row" sx={{ alignItems: 'center', gap: 1 }}><BarChartOutlined style={{ color: theme.vars.palette.text.secondary }} /><Typography variant="caption" color="text.secondary">Manage each monitoring job independently from Domains, SSL Certificates, and Webspace in the Website &amp; Hosting section.</Typography></Stack></Grid>
    </Grid>
  );
}
