...
export default function ApplicationActionButtons({ application }) {
  const { performAction, deploy, runGitPull, runBuild, pendingActions } = usePm2();
  const [action, setAction] = useState(null);
  const label = action === 'stop' ? 'Stop Application' : action === 'restart' ? 'Restart' : action === 'deploy' ? 'Deploy' : action === 'git-pull' ? 'Git Pull Origin' : 'Build';
  const pendingAction = pendingActions[application.id];
  const isPending = Boolean(pendingAction);
  const pendingLabel = pendingAction === 'restart' ? 'Restarting...' : pendingAction === 'stop' ? 'Stopping...' : pendingAction === 'git-pull' ? 'Pulling...' : pendingAction === 'build' ? 'Building...' : 'Deploying...';
  return (
    <>
